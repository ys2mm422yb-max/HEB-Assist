...
function updateEarlyStartupUi(status = {}) {
  ...
  if (status.status === 'loading') {
    ...
    ui.title.textContent = percent >= 97 ? 'Lokale KI wird gestartet' : 'Lokale KI wird geladen';
    ui.text.textContent = percent >= 97
      ? 'Das Sprachmodell wird jetzt auf diesem Gerät initialisiert.'
...
async function startLocalAiImmediately() {
  ...
  aiStartupPromise = (async () => {
    ...
    const capability = aiModule.getLocalAiCapability?.();
    if (!capability?.supported) return;
    await aiModule.preloadLocalAi(updateEarlyStartupUi);
  })().catch((error) => {
    updateEarlyStartupUi({
      status: 'error',
      percent: 0,
...
void startApp().finally(() => {
  void prepareServiceWorkerAfterAiStart();
});
...
document.addEventListener('visibilitychange', () => {
  if (!document.hidden) void checkForAppUpdate();
});
